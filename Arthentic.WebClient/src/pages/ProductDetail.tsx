  ...
  useEffect(() => {
    ...
    const fetchDetail = async () => {
      try {
        ...
        // Dùng relative URL (tốt nhất khi đã config proxy)
        const res = await axios.get(`/api/paintings/${id}`);
        
        console.log("✅ API trả về thành công:", res.data);
        setPainting(res.data);
      } catch (err: any) {
        console.error("❌ Network Error:", err);
        setError("Không thể kết nối với server. Backend có đang chạy không?");
      } finally {
        setLoading(false);
      }
    };
    ...
    <div className="max-w-6xl mx-auto px-6 py-12">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
        ...
        <div>
          ...
          <p className="text-xl text-gray-600 mt-2">bởi <span className="font-semibold">{painting.artistName}</span></p>
